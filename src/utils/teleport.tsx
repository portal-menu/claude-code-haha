...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    ...
    if (!accessToken) {
      ...
      throw new Error('Open Claude Code 中文汉化版 网页会话需要使用 Claude.ai 账户进行身份验证。API 密钥身份验证不足。请运行 /login 进行身份验证，或使用 /status 检查您的身份验证状态。');
    }
